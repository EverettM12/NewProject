...
func _ready() -> void:
	...
	if SaveManager.save_data.Shrine_2_opened:
		door_anim.play("Open")
	else:
...
func _on_body_entered_pedestal(body: Node3D) -> void: # Pedestal
	if body.is_in_group("Player") and can_interact == true and SaveManager.save_data.Shrine_2_opened == false and body is playercontroller:
...
func _input(event: InputEvent) -> void:
	#===============================================================================
	if event.is_action_pressed("Interact") and pedestal_ui.visible == true and SaveManager.save_data.Shrine_2_opened == false: # Pedestal
		can_interact = false
		door_anim.play("Open_Door")

	#===============================================================================
	if event.is_action_pressed("Interact") and is_on_platform == true:
...
func shrine_has_opened():
	SaveManager.load()
	SaveManager.save_data.Shrine_2_opened = true
	SaveManager.save()
	print(shrine_name, " has opened!")

func _on_shrine_discovered(body: Node3D) -> void:
	if body.is_in_group("Player") and body is playercontroller:
		if DisplayText and DisplayText.label and SaveManager.save_data.Shrine_2_text_played == false:
			DisplayText.label.text = shrine_name
			DisplayText.sub_label.text = shrine_sub_name
			DisplayText.appear()
			
			SaveManager.load()
			SaveManager.save_data.Shrine_2_text_played = true
			SaveManager.save()
